...
Modified some API to fix carla 098.

eg: waypoint is junction API
...
def get_waypoint_in_distance(waypoint, distance):
    ...
    """
    traveled_distance = 0
    sample_radius = 0.1  # use a sample radius for a more accurate result
    while not waypoint.is_junction and traveled_distance < distance:
        waypoint_new = waypoint.next(sample_radius)[-1]
        traveled_distance += waypoint_new.transform.location.distance(waypoint.transform.location)
        waypoint = waypoint_new
...
def generate_target_waypoint(waypoint, turn=0):
    ...
    while True:
        ...
        #   End condition for the behavior
        # if reach into the straight lane period
        if turn != 0 and reached_junction and len(wp_list) >= 3:
            ...
            # fix the condition criteria
            # if angle_wp < threshold:
            #     break

            if angle_wp < threshold and not wp_list[-1].is_junction:
                break
