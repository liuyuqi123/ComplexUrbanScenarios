...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, carla_map: carla.Map = None, opt_dict=None):
        """
        :param vehicle: actor to apply to local planner logic onto
        :param carla_map: carla.Map, to improve code efficiency
    ...
    def _init_controller(self, opt_dict):
        ...
        self._max_brake = 0.75  # 0.3
        self._max_throt = 1.0  # 0.75
        self._max_steer = 0.5  # 0.8
        args_lateral_dict = {
            'K_P': 1.95,
            'K_D': 0.2,
            'K_I': 0.07,
    ...
    def run_step(self, debug=False):
        ...
        target_wp_tuple = (self.target_waypoint, self._target_road_option)

        # move using PID controllers
        # control = self._vehicle_controller.run_step(self._target_speed, self.target_waypoint)

        control = self._vehicle_controller.run_step(self._target_speed, target_wp_tuple)
        ...
        for i, (transform, _) in enumerate(self._waypoint_buffer):
            if transform.location.distance(vehicle_transform.location) < self._min_distance:
                max_index = i
